export async function onRequestGet(context) {
  try {
    ...
    if (!longUrl) {
      return new Response(
        JSON.stringify({ ok: false, shortUrl: null, originalUrl: '', error: 'Missing url parameter' }),
    ...
    return new Response(
      JSON.stringify({ ok: looksLikeUrl, shortUrl: looksLikeUrl ? text : null, originalUrl: longUrl, raw: text }),
      { status: 200, headers: { 'Content-Type': 'application/json; charset=utf-8', 'Cache-Control': 'no-store' } }
    );

  } catch (err) {
    return new Response(
      JSON.stringify({ ok: false, shortUrl: null, originalUrl: '', error: err?.message || 'Shortening failed' }),
